...
namespace {
  ...
  const char* labels[label_count] = {"fetch", "left", "right", "spin", "stop", "up"};
  int mode = 1;
...
void loop() {
  ...
  if ( mode == 1 && receivedData == '1'){
    Serial.println("Swithcing to mode 2: Magic Wand Mode!!");
    mode = 2;
  ...
  if (mode == 1) {
      ...
      if (receivedData == '2') {
        Serial.println("left");
	      connectToPeripheral(2);
	    }
      if (receivedData == '3') {
        Serial.println("spin");
	      connectToPeripheral(4);
	    }
      if (receivedData == '4') {
        Serial.println("right");
	      connectToPeripheral(3);
	    }
      if (receivedData == '5') {
        Serial.println("stop");
	      connectToPeripheral(6);
	    }
      if (receivedData == '6') {
        Serial.println("up");
	      connectToPeripheral(5);
	    }
	  }
    else{
  ...
  //Serial.println("Note: Recording in progress");
  ...
  if (done_just_triggered) {
    Serial.println("Note: Recording Done! ");
    ...
    Serial.println("Note: Paasing to Model");
    TfLiteTensor* model_input = interpreter->input(0);
    ...
    connectToPeripheral(max_index);
    if (max_index == 4){
      mode = 1;
      Serial.println("Swithcing to mode 1: KWS Mode!!");
    }
...
void connectToPeripheral(int max_index){
  Serial.print(max_index);
if (peripheral.connected()){
  controlPeripheral(peripheral, max_index);
}else {
...
void controlPeripheral(BLEDevice peripheral, int max_index) {
  Serial.println("- Connecting to peripheral device...");
  if (peripheral.connected()){
    gestureCharacteristic.writeValue((byte)max_index);
  ...
  while (peripheral.connected()) {
      if (max_index == 0) {
        gestureCharacteristic.writeValue((byte)6);
      }
      else{
      gestureCharacteristic.writeValue((byte)max_index);}
      Serial.println("* Writing value to gesture_type characteristic done!");
